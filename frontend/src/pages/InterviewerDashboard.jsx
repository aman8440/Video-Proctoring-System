...
const InterviewerDashboard = () => {
  ...
  const setupSocketIO = () => {
    try {
      ...
      socketRef.current.on('connect', () => {
        setIsConnected(true);
        console.log('Socket.IO connected - Interviewer dashboard');
        
        // Join the session room with role
      ...
      socketRef.current.on('reconnect', (attemptNumber) => {
        ...
        setIsConnected(true);
        // Rejoin the session room after reconnection
        socketRef.current.emit('join-session', {
          sessionId,
          role: 'interviewer'
        });
  ...
  const handleRealTimeAlert = (alertData) => {
    ...
    setSessionStats(prev => ({
      ...
      integrityScore: Math.max(0, prev.integrityScore - 5), // Decrease integrity score
      alertsByType: {
        ...prev.alertsByType,
    ...
    try {
      const response = await fetch(`https://video-proctoring-system-52ph.onrender.com/api/sessions/${sessionId}/end`, {
        ...
        body: JSON.stringify({
          ...
          finalIntegrityScore: sessionStats.integrityScore
        })
      });

      if (response.ok) {
        // Notify candidate that session has ended
